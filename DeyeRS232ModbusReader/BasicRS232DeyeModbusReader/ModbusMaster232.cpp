...
/*
  ...
  ModbusMaster232.cpp - Arduino library for communicating with Modbus slaves
  over RS232/485 (via RTU protocol).

  Library:: ModbusMaster232
...
int OnRxFunction = 0;
uint8_t u8ModbusPollADU[256];
uint8_t u8ModbusPollADUSize = 0;
uint8_t u8PollBytesLeft = 8;
uint8_t PollBytesRX = 0;
uint32_t PollLastReadTime;
uint8_t PollMBStatus = 0;
int PollGotLen = 0;
uint8_t u8MBPollFunction;
#define WAIT_RX       1
#define WAIT_RX_REST  2
static int PollMode = WAIT_RX;
  
/* _____PUBLIC FUNCTIONS_____________________________________________________ */
/**
Constructor.

Creates class object; initialize it using ModbusMaster232::begin().
...
@ingroup setup
*/
void ModbusMaster232::begin(uint8_t slave, Stream &serial)
{
...
buffer that is used by ModbusMaster232. Use of i2c/TWI, 1-Wire, other
serial ports, etc. is permitted within callback function.

@see ModbusMaster232::ModbusMasterTransaction()
*/
...
@see ModbusMaster232::ModbusMasterTransaction()
@see ModbusMaster232::postTransmission()
*/
void ModbusMaster232::preTransmission(void (*preTransmission)())
{
...
@see ModbusMaster232::ModbusMasterTransaction()
@see ModbusMaster232::preTransmission()
*/
void ModbusMaster232::postTransmission(void (*postTransmission)())
{
...
@see ModbusMaster232::clearResponseBuffer()
...
@see ModbusMaster232::getResponseBuffer(uint8_t u8Index)
@ingroup buffer
*/
...
@see ModbusMaster232::clearTransmitBuffer()
...
@see ModbusMaster232::setTransmitBuffer(uint8_t u8Index, uint16_t u16Value)
@ingroup buffer
*/
...
uint8_t ModbusMaster232::readPollHoldingRegisters(uint16_t u16ReadAddress,
  uint16_t u16ReadQty)
{
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;
  return ModbusMasterPollTransaction(ku8MBReadHoldingRegisters);
}
...
uint8_t ModbusMaster232::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  //while (!u8MBStatus)
  {
    ...
    if ((millis() - LastReadTime) > ku16MBResponseTimeout)
    {
    ...
    switch(u8ModbusADU[1])
    {
      ...
      default:
        for (i = 0; i < (u8ModbusADUSize  >> 1); i++)
        {
          if (i < ku8MaxBufferSize)
          {
            _u16ResponseBuffer[i] = word(u8ModbusADU[2 * i + 1], u8ModbusADU[2 * i + 2]);
          }
...
void ModbusMaster232::RxCallback(void)
{
    while (_serial->available())
    {
#if __MODdBUSMASTER_DEBUG__
      digitalWrite(__MODBUSMASTER_DEBUG_PIN_A__, true);
#endif
      u8ModbusPollADU[u8ModbusPollADUSize] = _serial->read();
      u8ModbusPollADUSize++;
      PollBytesRX++;
      PollLastReadTime = millis();
#if __MODBUSMASTER_DEBUG__
      digitalWrite(__MODBUSMASTER_DEBUG_PIN_A__, false);
#endif
    }  
...
uint8_t ModbusMaster232::ModbusMasterPollTransaction(uint8_t u8MBFunction)
{
  uint8_t i, u8Qty;
  uint16_t u16CRC;
  uint8_t u8MBStatus = ku8MBSuccess;
  
  u8ModbusPollADUSize = 0;
  // assemble Modbus Request Application Data Unit
  u8ModbusPollADU[u8ModbusPollADUSize++] = _u8MBSlave;
  u8ModbusPollADU[u8ModbusPollADUSize++] = u8MBFunction;
  u8MBPollFunction = u8MBFunction;
  ...
  switch(u8MBFunction)
  {
    ...
    case ku8MBReadHoldingRegisters:
    case ku8MBReadWriteMultipleRegisters:
      u8ModbusPollADU[u8ModbusPollADUSize++] = highByte(_u16ReadAddress);
      u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16ReadAddress);
      u8ModbusPollADU[u8ModbusPollADUSize++] = highByte(_u16ReadQty);
      u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16ReadQty);
      break;
  }
  ...
  switch(u8MBFunction)
  {
    ...
    case ku8MBWriteMultipleRegisters:
    case ku8MBReadWriteMultipleRegisters:
      u8ModbusPollADU[u8ModbusPollADUSize++] = highByte(_u16WriteAddress);
      u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16WriteAddress);
      break;
  }
  ...
  {
    case ku8MBWriteSingleCoil:
      u8ModbusPollADU[u8ModbusPollADUSize++] = highByte(_u16WriteQty);
      u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16WriteQty);
      break;
      
    case ku8MBWriteSingleRegister:
      u8ModbusPollADU[u8ModbusPollADUSize++] = highByte(_u16TransmitBuffer[0]);
      u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16TransmitBuffer[0]);
      break;
    ...
    case ku8MBWriteMultipleCoils:
      ...
      u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16WriteQty);
      u8Qty = (_u16WriteQty % 8) ? ((_u16WriteQty >> 3) + 1) : (_u16WriteQty >> 3);
      u8ModbusPollADU[u8ModbusPollADUSize++] = u8Qty;
      for (i = 0; i < u8Qty; i++)
      {
        switch(i % 2)
        {
          case 0: // i is even
            u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16TransmitBuffer[i >> 1]);
            break;
            
          case 1: // i is odd
            u8ModbusPollADU[u8ModbusPollADUSize++] = highByte(_u16TransmitBuffer[i >> 1]);
            break;
        }
      }
      break;
    ...
    case ku8MBReadWriteMultipleRegisters:
      u8ModbusPollADU[u8ModbusPollADUSize++] = highByte(_u16WriteQty);
      u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16WriteQty);
      u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16WriteQty << 1);
      
      for (i = 0; i < lowByte(_u16WriteQty); i++)
      {
        u8ModbusPollADU[u8ModbusPollADUSize++] = highByte(_u16TransmitBuffer[i]);
        u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16TransmitBuffer[i]);
      }
      break;
    ...
    case ku8MBMaskWriteRegister:
      u8ModbusPollADU[u8ModbusPollADUSize++] = highByte(_u16TransmitBuffer[0]);
      u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16TransmitBuffer[0]);
      u8ModbusPollADU[u8ModbusPollADUSize++] = highByte(_u16TransmitBuffer[1]);
      u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(_u16TransmitBuffer[1]);
      break;
  }
  
  // append CRC
  u16CRC = 0xFFFF;
  for (i = 0; i < u8ModbusPollADUSize; i++)
  {
    u16CRC = crc16_update(u16CRC, u8ModbusPollADU[i]);
  }
  u8ModbusPollADU[u8ModbusPollADUSize++] = lowByte(u16CRC);
  u8ModbusPollADU[u8ModbusPollADUSize++] = highByte(u16CRC);
  u8ModbusPollADU[u8ModbusPollADUSize] = 0;
  ...
  for (i = 0; i < u8ModbusPollADUSize; i++)
  {
    _serial->write(u8ModbusPollADU[i]);
  }
  ...
  PollMode = WAIT_RX;
  PollLastReadTime = millis();
  PollMBStatus = ku8MBSuccess;
  return u8MBStatus;
}
  
uint8_t ModbusMaster232::PollModbus(void)
{
uint16_t u16CRC;
int i;
  ...
  switch(PollMode)
  {
    case WAIT_RX :  
      ...
      if ((millis() - PollLastReadTime) > ku16MBResponseTimeout)
      {
        PollMBStatus = ku8MBResponseTimedOut;
        PollMode = WAIT_RX;
        break;
      }
      if(PollBytesRX >= u8PollBytesLeft)
      {
        // evaluate slave ID, function code once enough bytes have been read
        if(PollGotLen == 0)
        {
          // verify response is for correct Modbus slave
          if (u8ModbusPollADU[0] != _u8MBSlave)
          {
            PollMBStatus = ku8MBInvalidSlaveID;
            break;
          }
          
          // verify response is for correct Modbus function code (mask exception bit 7)
          if ((u8ModbusPollADU[1] & 0x7F) != u8MBPollFunction)
          {
            PollMBStatus = ku8MBInvalidFunction;
            break;
          }
          
          // check whether Modbus exception occurred; return Modbus Exception Code
          if (bitRead(u8ModbusPollADU[1], 7))
          {
            PollMBStatus = u8ModbusPollADU[2];
            break;
          }
          
          // evaluate returned Modbus function code
          switch(u8ModbusPollADU[1])
          {
            case ku8MBReadCoils:
            case ku8MBReadDiscreteInputs:
            case ku8MBReadInputRegisters:
            case ku8MBReadHoldingRegisters:
            case ku8MBReadWriteMultipleRegisters:
              u8PollBytesLeft = u8ModbusPollADU[2];
              break;
          ...
          PollGotLen = 1;
          PollMode = WAIT_RX_REST; 
        }
      }
      if(!PollMBStatus)
        return(ku8MBPollWaitRx);
      break;
      
    case WAIT_RX_REST :
      if(PollBytesRX < (u8PollBytesLeft + 5))
      {
        if ((millis() - PollLastReadTime) > ku16MBResponseTimeout)
        {
          PollMBStatus = ku8MBResponseTimedOut;
          ...
          return(ku8MBPollWaitRest);
      }
        
      PollMode = WAIT_RX;
      // verify response is large enough to inspect further
      if (!PollMBStatus && u8ModbusPollADUSize >= 5)
      {
        // calculate CRC
        u16CRC = 0xFFFF;
        for (i = 0; i < (u8ModbusPollADUSize - 2); i++)
        {
          u16CRC = crc16_update(u16CRC, u8ModbusPollADU[i]);
        }
        
        // verify CRC
        if (!PollMBStatus && (lowByte(u16CRC) != u8ModbusPollADU[u8ModbusPollADUSize - 2] ||
          highByte(u16CRC) != u8ModbusPollADU[u8ModbusPollADUSize - 1]))
        {
          PollMBStatus = ku8MBInvalidCRC;
        }
      }
      // disassemble ADU into words
      if (!PollMBStatus)
      {
        ...
        switch(u8ModbusPollADU[1])
        {
          ...
          case ku8MBReadDiscreteInputs:
            // load bytes into word; response bytes are ordered L, H, L, H, ...
            for (i = 0; i < (u8ModbusPollADU[2] >> 1); i++)
            {
              if (i < ku8MaxBufferSize)
              {
                _u16ResponseBuffer[i] = word(u8ModbusPollADU[2 * i + 4], u8ModbusPollADU[2 * i + 3]);
              }
            ...
            // in the event of an odd number of bytes, load last byte into zero-padded word
            if (u8ModbusPollADU[2] % 2)
            {
              if (i < ku8MaxBufferSize)
              {
                _u16ResponseBuffer[i] = word(0, u8ModbusPollADU[2 * i + 3]);
              }
          ...
          case ku8MBReadWriteMultipleRegisters:
            ...
            for (i = 0; i < (u8ModbusPollADU[2] >> 1); i++)
            {
              if (i < ku8MaxBufferSize)
              {
                _u16ResponseBuffer[i] = word(u8ModbusPollADU[2 * i + 3], u8ModbusPollADU[2 * i + 4]);
              }
          ...
          default:
            for (i = 0; i < (u8ModbusPollADUSize  >> 1); i++)
            {
              if (i < ku8MaxBufferSize)
              {
                _u16ResponseBuffer[i] = word(u8ModbusPollADU[2 * i + 1], u8ModbusPollADU[2 * i + 2]);
              }
