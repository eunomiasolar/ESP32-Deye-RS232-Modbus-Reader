...
//extern "C" {
extern int FtpReadFileUpCall(unsigned char * Buffer, int Len);
extern int FtpWriteFileUpCall(unsigned char * Buffer, int Len);
//}
...
void ESP32_FTPClient::WriteClientUpCall(void)
{
  ...
  Serial.printf("FTP : Write\n");

  if(!isConnected()) 
    return;

  Serial.printf("FTP : Connected\n");

  cli = GetDataClient();
  while(1)
  {
    //count = ReadCall(&clientBuf[0], 512);
    count = FtpReadFileUpCall(&clientBuf[0], 512);
    Serial.printf("FTP : Request 512 bytes, got %d\n", count);
    if(count < 512) 
    {
      if(count > 0) 
...
{
  Serial.printf("FTP : ServerAdress %s\n", serverAdress);
...
int ESP32_FTPClient::DownloadFileStart(const char * filename) 
{
char *ptr;
int length = 0xFFFFFFFF;
  ...
  //Resp = [150 Opening BINARY mode data connection for LBfirmware.bin (837872 bytes).
...
void ESP32_FTPClient::DownloadFile(void) 
{
int len, count;
  ...
  while( !dclient.available() && (millis() < _m + timeout)) 
    delay(1);
  Serial.printf("Waited %d millis\n", millis() - _m);

  while(dclient.available()) 
  {
    len = dclient.readBytes(&clientBuf[0], sizeof(clientBuf));
    //Serial.printf("Read %d\n", len);
    count = FtpWriteFileUpCall(&clientBuf[0], len);
    if(len < sizeof(clientBuf))
    {
      sleep(1);
      break;
    }
