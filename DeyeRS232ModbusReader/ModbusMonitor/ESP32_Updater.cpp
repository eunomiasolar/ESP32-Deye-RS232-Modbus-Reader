...
bool ESP32_UpdateClass::_enablePartition(const esp_partition_t* partition)
{
    if(!partition)
    {
        return false;
...
ESP32_UpdateClass::ESP32_UpdateClass()
: _error(0)
, _buffer(0)
, _bufferLen(0)
, _size(0)
...
ESP32_UpdateClass& ESP32_UpdateClass::onProgress(THandlerFunction_Progress fn) 
{
    _progress_callback = fn;
    return *this;
}

void ESP32_UpdateClass::_reset() 
{
    if (_buffer)
        delete[] _buffer;
    _buffer = 0;
...
bool ESP32_UpdateClass::canRollBack()
{
    if(_buffer)
    { //Update is running
        return false;
...
bool ESP32_UpdateClass::rollBack()
{
    if(_buffer)
    { //Update is running
        return false;
...
bool ESP32_UpdateClass::begin(size_t size, int command, int ledPin, uint8_t ledOn, const char *label) 
{
    if(_size > 0)
    {
        Serial.printf("already running\n");
        return false;
    }
...
void ESP32_UpdateClass::_abort(uint8_t err)
{
    _reset();
    _error = err;
}

void ESP32_UpdateClass::abort()
{
    _abort(UPDATE_ERROR_ABORT);
}

bool ESP32_UpdateClass::_writeBuffer()
{
...
bool ESP32_UpdateClass::_verifyHeader(uint8_t data) 
{
    if(_command == U_FLASH) 
    {
...
bool ESP32_UpdateClass::_verifyEnd() 
{
    if(_command == U_FLASH) 
    {
...
bool ESP32_UpdateClass::setMD5(const char * expected_md5)
{
...
bool ESP32_UpdateClass::end(bool evenIfRemaining)
{
    if(hasError() || _size == 0)
    {
        return false;
    ...
    {
        Serial.printf("premature end: res:%u, pos:%u/%u\n", getError(), progress(), _size);
        _abort(UPDATE_ERROR_ABORT);
        return false;
    }
...
size_t ESP32_UpdateClass::write(uint8_t *data, size_t len) 
{
...
size_t ESP32_UpdateClass::writeStream(Stream &data) 
{
    size_t written = 0;
    size_t toRead = 0;
...
void ESP32_UpdateClass::printError(Print &out)
{
    out.println(_err2str(_error));
}

const char * ESP32_UpdateClass::errorString()
{
    return _err2str(_error);
}

bool ESP32_UpdateClass::_chkDataInBlock(const uint8_t *data, size_t len) const {
