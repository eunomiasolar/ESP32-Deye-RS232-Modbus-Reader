...
uint8_t ModbusMaster232::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  if(Debug) Serial.printf("TX ");
  for (i = 0; i < u8ModbusADUSize; i++)
  {
    if(Debug) Serial.printf("%02X,", u8ModbusADU[i]);
  }
  ...
  //while (!u8MBStatus)
  {
    ...
    if((u8ModbusADUSize == 5) && (GotLen == 0))
    {
      ...
      if (u8ModbusADU[0] != _u8MBSlave)
      {
        ...
        if(Debug) Serial.printf("InvalidSlaveID" );
        break;
      }
      ...
      if ((u8ModbusADU[1] & 0x7F) != u8MBFunction)
      {
        ...
        if(Debug) Serial.printf("Fn err " );
        break;
      }
      if(Debug) Serial.printf("Fn %02X ", (u8ModbusADU[1] & 0x7F) );
      
      // check whether Modbus exception occurred; return Modbus Exception Code
      if (bitRead(u8ModbusADU[1], 7))
      {
        u8MBStatus = u8ModbusADU[2];
        if(Debug) Serial.printf("Exception ");
        break;
      }
      ...
      switch(u8ModbusADU[1])
      {
        ...
        case ku8MBReadWriteMultipleRegisters:
          ...
          //if(Debug) Serial.printf("RdLen %02X ", u8BytesLeft);
          break;
        ...
        case ku8MBWriteMultipleRegisters:
          u8BytesLeft = 3;
          //if(Debug) Serial.printf("WrLen %02X ", u8BytesLeft);
          break;
          
        case ku8MBMaskWriteRegister:
          u8BytesLeft = 5;
          //if(Debug) Serial.printf("RgLen %02X ", u8BytesLeft);
          break;
      }
      GotLen = 1;
    }
    ...
    {
      if(Debug) Serial.printf("Timeout ");
...
//    {
//      More = _serial->read();
//      if(Debug) Serial.printf("+%02X", More);
//    }
//    if(Debug) Serial.printf("\n");
//  }
  ...
  if (!u8MBStatus && u8ModbusADUSize >= 5)
  {
    ...
    //if(Debug) Serial.printf("CRC %04X ", u16CRC);
  ...
  if (!u8MBStatus)
  {
    ...
    switch(u8ModbusADU[1])
    {
      ...
      case ku8MBReadDiscreteInputs:
        ...
        //if(Debug) Serial.printf("Packed %d discretes", _u8ResponseBufferLength );
        break;
      ...
      case ku8MBReadWriteMultipleRegisters:
        ...
        //if(Debug) Serial.printf("Packed %d registers", i);
        break;
